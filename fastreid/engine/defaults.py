...
from . import hooks
from .train_loop import SimpleTrainer, FixedTrainer

__all__ = ["default_argument_parser", "default_setup", "DefaultPredictor", "DefaultTrainer", "CenterTrainer"]
...
class CenterTrainer(FixedTrainer):
    """
    A trainer with default training logic. Compared to `SimpleTrainer`, it
    contains the following logic in addition:
    1. Create model, optimizer, scheduler, dataloader from the given config.
    ...
    def __init__(self, cfg):
        ...
        super().__init__(model, data_loader, optimizer, cfg.SOLVER.BASE_LR, cfg.MODEL.LOSSES.CENTER.LR, cfg.MODEL.LOSSES.CENTER.SCALE, cfg.SOLVER.AMP_ENABLED)
